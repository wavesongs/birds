...
class Paths(object):
    #%%
    def __init__(self, root_path="../examples/", audios_path='audios', 
                 results="results", catalog=False, spreadsheet="spreadsheet.csv"):
        self.root     = Path(root_path)
        self.catalog  = catalog
        self.audios   = self.root / audios_path
        self.results  = self.root / results
        self.MG_param = self.results / "MG"
        self.images   = self.results / "Images" # Result images folder
        self.examples = self.results / 'Audios' # Audios folder
        ...
        if catalog is True:
            data = pd.read_csv(self.audios/self.spreadsheet, encoding_errors='ignore')
    ...
    def AudioFiles(self, verbose=False):
        if verbose:
            ...
            for file in self.files_names: print("  "+file)
