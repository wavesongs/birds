...
class Plotter(object): 
    ...
    #%%
    def PlotAlphaBeta(self, obj, xlim=(-0.05,.2), ylim=(-0.2,0.9), figsize=(10,6), save=None):
        if "synth" in obj.file_name.split("-"):
            ...
            fig.suptitle("Motor Gesture Curves", fontsize=18, y=0.99, fontweight="bold")
            plt.text(x=0.5, y=0.92, s=obj.file_name[:-6], fontsize=12, ha="center", transform=fig.transFigure)
            plt.text(x=0.5, y=0.89, s="Type: {} - Syllable Number: {}".format(obj.type, obj.no_syllable), fontsize=10, ha="center", transform=fig.transFigure)
            #fig.tight_layout()
            plt.show()
            if save is None: 
                save = self.save
            if save: 
                fig.savefig(obj.paths.images / "{}-{}-{}-MotorGesturesParameters.png".format(obj.file_name,obj.id,obj.no_syllable), transparent=True, bbox_inches='tight')
            
            #return fig, gs
        else: print("This  is not a synthetic syllable, remember create a synthetic file using the funcion bs.Solve().")
    
    #%%
    def PlotVs(self, obj, xlim=(), figsize=(10,6), save=None):
        if "synth" in obj.file_name.split("-"):
            plt.close()
            ...
            ax[0,0].plot(obj.timesVs, obj.Vs[:,1]/1e3, color='g') 
            ax[0,0].set_ylabel("$p_{in}$ (x $10^3$)")
            ax[0,0].set_title(r"Trachea Input Pressure")
            ax[0,0].set_xlim(xlim)
            
            ax[0,1].plot(obj.timesVs, obj.Vs[:,4], color='b')
            ax[0,1].set_ylabel("$p_{out}$")
            ax[0,1].set_title(r"Trachea Output Pressure")
            ax[0,1].set_xlim(xlim)
            
            ax[1,0].plot(obj.timesVs, obj.Vs[:,0], color='r')
            ax[1,0].set_xlabel("Time (s)")
            ax[1,0].set_ylabel("$x(t)$")
            ax[1,0].set_title(r"Labial Walls Displacement")
            ax[1,0].set_xlim(xlim)
            
            ax[1,1].plot(obj.timesVs, obj.Vs[:,0], color='m')
            ax[1,1].set_xlabel("Time (s)")
            ax[1,1].set_ylabel("$y(t)$")
            ax[1,1].set_title(r"Labial Walls Velocity")
            ax[1,1].set_xlim(xlim)

            fig.suptitle("Physical Model Variables", fontsize=18, y=0.99, fontweight="bold")
            plt.text(x=0.5, y=0.91, s=obj.file_name[:-6], fontsize=12, ha="center", transform=fig.transFigure)
            plt.text(x=0.5, y=0.88, s="Type: {} - Syllable Number: {}".format(obj.type, obj.no_syllable), fontsize=10, ha="center", transform=fig.transFigure)
    ...
    def Plot(self, obj, syllable=None, chunck=None, FF_on=False, SelectTime_on=False, xlim=None, figsize=None, save=None): 
        ...
        if "birdsong" in obj.id:            
            ...
            ax[0].legend(bbox_to_anchor=(1.01, 0.75))
            ...
            fig.suptitle("Waveform and Spectrogram", fontsize=18, y=0.99, fontweight="bold")
            plt.text(x=0.5, y=0.91, s=obj.file_name[:-4], fontsize=12, ha="center", transform=fig.transFigure)
            ...
            fig.suptitle("Waveform and Spectrogram", fontsize=18, y=0.99, fontweight="bold")
            plt.text(x=0.5, y=0.9, s=obj.file_name[:-4], fontsize=12, ha="center", transform=fig.transFigure)
            plt.text(x=0.5, y=0.85, s="Type: {} - Syllable Number: {}".format(obj.type, obj.no_syllable), fontsize=10, ha="center", transform=fig.transFigure)

            fig.tight_layout()
            plt.subplots_adjust(wspace=0, hspace=0.1, top=0.8)
            plt.show()
    ...
    #%%    
    def Syllables(self, obj, obj_synth, FF_on=False, figsize=(10,6), save=None): 
        plt.close()
        ...
        ax[1,1].plot(obj_synth.time_s, obj_synth.envelope, label='envelope', c='#08181C')
        ...
        cbar_ax = fig.add_axes([0.915, 0.475, 0.015, 0.35])
        ...
        fig.suptitle("Comparing Syllables", fontsize=18, y=0.99, fontweight="bold")
        plt.text(x=0.5, y=0.92, s=obj.file_name[:-3], fontsize=12, ha="center", transform=fig.transFigure)
        plt.text(x=0.5, y=0.89, s="Type: {} - Syllable Number: {}".format(obj.type, obj.no_syllable), fontsize=10, ha="center", transform=fig.transFigure)
        ...
        if save is None: save = self.save
        if save: 
            fig.savefig(obj.paths.images / "{}-{}-{}-SoundAndSpectros.png".format(obj.file_name[:-4],obj.id,obj.no_syllable), transparent=True, bbox_inches='tight')
        
        #return fig, ax
    #%%
    def Scores(self, obj, obj_synth, figsize=(10,10), ylim=None, save=None):
        plt.close()
        ticks = ticker.FuncFormatter(lambda x, pos: '{0:g}'.format(x*1e-3))
        ...
        gs  = fig.add_gridspec(nrows=9, ncols=5, right=0.7, hspace=0.7, top=0.9)
        ...
        ax2.legend(bbox_to_anchor=(1.075, 0.95), borderpad=0.6, labelspacing=0.7,)
        ...
        ax11.set_ylim((0,5))
        ax11.legend(bbox_to_anchor=(1.28, 1))
        
        lns = lr+ls+lh+lskl+lc
        labs = [l.get_label() for l in lns]
        ax11.legend(lns, labs, bbox_to_anchor=(1.075, 1.1), title="Acoustical Features", title_fontproperties={'weight':'bold'})

        # ------------------ sound -------------------------
        ax9 = fig.add_subplot(gs[7:,:], sharex=ax2)
        ax9.plot(obj.time_s,       obj.s,              c='blue',  label=r'waveform$_{real}$', alpha=0.7)
        ax9.plot(obj_synth.time_s, obj_synth.s,        c='green', label=r'waveform$_{synth}$', alpha=0.7)
        ...
        ax9.legend(bbox_to_anchor=(1.075, 0.95))
        ax9.set_ylabel("Amplitude (a.u.)")
        
        cbar_ax = fig.add_axes([0.73, 0.477, 0.02, 0.24])
        clb = fig.colorbar(img, cax=cbar_ax)
        clb.set_label('Power (dB)', labelpad=-20, y=1.125, rotation=0)
        
        fig.suptitle("Scoring Variables", fontsize=18, y=0.99, fontweight="bold")
        plt.text(x=0.5, y=0.95, s=obj.file_name[:-3], fontsize=12, ha="center", transform=fig.transFigure)
        plt.text(x=0.5, y=0.93, s="Type: {} - Syllable Number: {}".format(obj.type, obj.no_syllable), fontsize=10, ha="center", transform=fig.transFigure)
        #fig.tight_layout()
        plt.show()
    ...
    def ComparingSpectros(self, obj, obj_synth, cmp="afmhot_r", figsize=None, ylim=None, save=None, sharey=True):
        ...
        fig.suptitle("Comparing Spectral Content", fontsize=18, y=0.99, fontweight="bold")
        plt.text(x=0.5, y=0.9, s=obj.file_name[:-3], fontsize=12, ha="center", transform=fig.transFigure)
        plt.text(x=0.5, y=0.85, s="Type: {} - Syllable Number: {}".format(obj.type, obj.no_syllable), fontsize=10, ha="center", transform=fig.transFigure)    
        plt.show()
    ...
    def SelectData(self, obj, FF_on=False, xlim=None, figsize=None, waveform=False, save=None): 
        ...
        fig.suptitle("Selecting Data\n{}"+obj.file_name[:-3], fontsize=18)
