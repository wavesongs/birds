...
class Optimizer(Syllable, object):
    ...
    def __init__(self, obj, method_kwargs):
        ...
        self.kwargs = {key:value for key, value in method_kwargs.items() if key!="method"}
        self.syllables = []
    #%%
    ...
    def OptimalBs(self, obj):
        ...
        end02   = time.time()
        print(r"$b_0*$"+"={:.4f},\nb_2*={:.4f}, t={:.2f} min".format(self.obj.p["b0"].value, self.obj.p["b2"].value, (end02-start02)/60))
        # ---------------- b1--------------------
        start1 = time.time()
        ...
        end1   = time.time()
        print(r"$b_1*$"+"={0:.4f}, t={1:.2f} min".format(self.obj.p["b1"].value, (end1-start1)/60))
    ...
    def OptimalBs_chunck(self, obj):
        ...
        end0   = time.time()
        print(r"$b_0*$"+"={0:.4f}, t={1:.2f} min".format(self.obj.p["b0"].value, (end0-start0)/60))
    # ---------------- b1--------------------
        start1 = time.time()
        self.obj.p["b1"].set(vary=True)
        mi1    = lmfit.minimize(self.residualFF, self.obj.p, nan_policy='omit', method=self.method, **self.kwargs) 
    ...
    def OptimalAs(self, obj):
        ...
        mi0    = lmfit.minimize(self.residualSCI, self.obj.p, nan_policy='omit', method=self.method, **self.kwargs) # self.residualCorrelation
        self.obj.p["a0"].set(vary=False, value=mi0.params["a0"].value)
        end0   = time.time()
        print(r"$a_0*$"+"={0:.4f}, t={1:.2f} min".format(self.obj.p["a0"].value, (end0-start0)/60))
        # ---------------- a1--------------------
        # start1 = time.time()
        ...
        # print(r"$a_1*$"+"={0:.4f}, t={1:.2f} min".format(self.obj.p["a1"].value, (end1-start1)/60))
        # #return self.obj.p["b0"].value, self.obj.p["b1"].value #end0-start0, end1-start1
        obj = self.obj
    #%%
    def OptimalGamma(self, obj):
        ...
        print("γ* =  {0:.0f}, t={1:.2f} min".format(self.obj.p["gm"].value, (end-start)/60))
    ...
    #%%
    def OptimalParams(self, obj, Ns=11):
        if self.method=="brute": self.kwargs["Ns"] = Ns     
        start = time.time()
